...
% alpha = 0.05;
% size = zeros(B,1);
% bootperint = zeros(B,2);
% mydata = chi2rnd(5,n,1);
% for i = 1:B
%     thetahat = mean(mydata);
%     [bootreps, bootsam] = bootstrp(B,'mean',mydata);
%     SE_mean = std(bootreps);
%     bval = sort(bootreps);
%     k = B*alpha/2;
%     bootperint(i,:) = [bval(k) bval(B-k)];
%     size(i) = bootperint(i,2)-bootperint(i,1);
% end
% size
n = 200;
alpha = 0.05;
...
% to a bootstrap sample.
   ...
   % Do bootstrap using that sample to estimate SE.
      sehats(i) = std(bootstrp(C,'mean',xstar));
  end
